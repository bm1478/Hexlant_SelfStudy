...
bitcoinSVRPC.getTransactionInfo(txId).then(result => {
    ...
    for(var i=0; i<myUTXO.length; i++) {
        ...
        const dataDestroy = async() => {
            await transactions.destroy({where: {txid: txId}});
        };
        //dataDestroy();

        const dataFind = async() => {
            const utxos = await transactions.findAll({});
            for (const utxo of utxos) console.log(`${utxo.address} / ${utxo.txid} / ${utxo.satoshis}`);
        };
        dataFind();
    }
